...
<div id="laimi-content" class="mui-content">
	<div class="mui-card mui-table-view mui-table-view-cell" style="margin:0px;padding:10px 12px;margin-top:6px;">
		...
		<div class="mui-media-body mui-navigate-right laimi-address" style="white-space:normal;">
			<a href="javascript:;">
				<span class="laimi-font14" style="font-weight:bold;line-height:24px;"><?php if(!empty($this->_data['address'])) echo $this->_data['address']['waddress_name']."，".$this->_data['address']['waddress_phone']; ?></span>&nbsp;&nbsp;<?php if(!empty($this->_data['address'])){if($this->_data['address']['waddress_state'] == 2){ ?><span class="laimi-font10" style="border:1px solid #FF5053;border-radius:3px;padding:1px 4px; color:#FF5053;">默认地址</span><?php }} ?>
			</a>
			<div class="laimi-color-gray laimi-font12" style="margin-right:30px;">
			<?php if(!empty($this->_data['address'])) echo $this->_data['address']['waddress_detail'];else echo "添加地址";  ?>
			</div>
		</div>
	</div>
	...
	<div class="mui-card mui-table-view mui-table-view-cell" style="margin:0px;padding:10px 12px;margin-top:8px">
		<img class="mui-media-object mui-pull-left" src="../upload/<?php echo api_value_int0($GLOBALS['_SESSION']['login_cid']); ?>/wgoods_image/<?php echo $row['photo'];?>"  style="max-width:75px;height:75px;border-radius:2px;">
	...
	<div class="mui-card" style="margin:0px;margin-top:6px;">
	    ...
	    <li class="mui-table-view-cell">
		    <a href="javascript:;" class="mui-navigate-right">
		    	<span class="mui-badge mui-badge-inverted <?php if($GLOBALS['gwshop']['send_method'] == 1) echo ' hide'; ?>" <?php if($GLOBALS['gwshop']['send_method'] != 2){ ?> style="margin-right:80px; <?php } ?>">
		    		<input class="laimi-gettype" name="gettype" type="radio" value="3"  <?php if($GLOBALS['gwshop']['send_method'] == 2) echo ' checked'; ?>>
		    		用户自取
		    	</span>
		    	<span class="mui-badge mui-badge-inverted <?php if($GLOBALS['gwshop']['send_method'] == 2) echo ' hide'; ?>">
		    		<input class="laimi-gettype" name="gettype" type="radio" value="2" <?php if($GLOBALS['gwshop']['send_method'] != 2) echo ' checked'; ?>>
		    		商城发货
		    	</span>
          	请选择取货方式
        </a>
