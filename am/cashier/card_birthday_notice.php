...
$gtemplate->fun_show('card_birthday_notice');
...
function get_card_list() {
	...
	$inttime = time();
	$inttime2 = strtotime('-3 day');
	$inttime3 = strtotime('+4 day');
	$intyear = date('Y', $inttime);

	$strwhere = '';
	$strhaving = '';
	if($GLOBALS['intstate'] == 1) {
		$strwhere .= " AND card_birthday_month = " . date('n', $inttime) . " AND card_birthday_day = " . date('j', $inttime);
	} else if($GLOBALS['intstate'] == 2) {
		//和今年的生日进行3天的比较
		$strhaving .= "  HAVING unix_timestamp(nowdate) >= " . $inttime2 . " AND unix_timestamp(nowdate) <= " . $inttime3;
	} else {
		$strwhere .= " AND card_birthday_month = " . date('n', $inttime) . " AND card_birthday_day = " . date('j', $inttime);
	}
	...
	$strwhere .= " GROUP BY card_id ";
	
	$arr = array();
	$strsql = "SELECT COUNT(card_id) AS mycount, CONCAT(".$intyear.", '-', card_birthday_month, '-', card_birthday_day) as nowdate FROM " . $GLOBALS['gdb']->fun_table2('card')  . " WHERE 1 = 1" . $strwhere . $strhaving;
	// die($strsql);
	$hresult = $GLOBALS['gdb']->fun_query($strsql);
	...
	$strsql = "SELECT a.*, b.shop_name FROM (SELECT CONCAT(".$intyear.", '-', card_birthday_month, '-', card_birthday_day) as nowdate, card_id, shop_id, card_okey, card_code, card_name, card_phone, card_sex, card_birthday_date, card_state, card_atime, card_edate, "
	...
	. " WHERE 1 = 1" . $strwhere . $strhaving . " ORDER BY card_id DESC LIMIT ". $intoffset . ", " . $intpagesize
