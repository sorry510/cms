...
$gtemplate->fun_show('store_goods_warning2');
...
function get_store_info_list() {
	...
	$strwhere = '';
	$strwhere .= " AND a.sgoods_id <> 0 AND a.shop_id = " . api_value_int0($GLOBALS['_SESSION']['login_sid']);
	if($GLOBALS['strkey'] != '') {
	...
	$strwhere .= " AND store_info_count <= ". $GLOBALS['gtrade']['store_count'];
	$arr = array();
	...
	$strsql = "SELECT a.store_info_id, a.store_info_count, b.sgoods_code, b.sgoods_name, b.sgoods_price, b.sgoods_cprice, c.sgoods_catalog_name FROM "
	. $GLOBALS['gdb']->fun_table2('store_info') . " AS a LEFT JOIN " . $GLOBALS['gdb']->fun_table2('sgoods') . " AS b ON a.sgoods_id = b.sgoods_id LEFT JOIN "
	. $GLOBALS['gdb']->fun_table2('sgoods_catalog') . " AS c ON b.sgoods_catalog_id = c.sgoods_catalog_id WHERE 1 = 1" . $strwhere
