...
$strshop_id = api_value_int0($GLOBALS['_SESSION']['login_sid']);
$intshop_id = api_value_int0($strshop_id);
$strgoods = api_value_get('goods_id');
...
function get_request() {
	...
	$arr['goods'] = $GLOBALS['strgoods'];
	$arr['count'] = $GLOBALS['strcount'];
...
function get_goods_count_list() {
	...
	if(!empty($GLOBALS['intcount'])) {
		if ($GLOBALS['intcount'] == 2) {
			$sqlorder = 'sales_count';
		}
	}
	...
	if ($GLOBALS['strgoods2'] != '') {
		...
		$intgoods_type = $arrgoods[0];
		$intgoods_id = $arrgoods[1];
		if ($intgoods_type == 1) {
	...
	$arr = array();
	$strsql = 'SELECT count(a.datcount1) as datcount FROM (SELECT count(card_record3_goods_id) as datcount1 FROM' . $GLOBALS['gdb']->fun_table2('card_record3_goods') . ' WHERE 1 = 1 ' . $strwhere .' GROUP BY mgoods_id,sgoods_id) as a';
