		  ...
		  <table class="layui-table" style="width: 90%;margin-left: 40px;">
			...
			<div class="layui-form-item">
				<div class="layui-input-block laimi-buttom-20">
					...
					<button id="laimi-self" type="button" class="layui-btn laimi-button-100 ">到店自取</button>
...
<?php echo $this->fun_fetch('inc_foot', ''); ?>
	...
	layui.use(["element", "laydate", "laypage", "upload", "layer", "form"], function() {
	    ...
	    var data = {
			...
			objlayer.open({
			    ...
			    $.getJSON(url,data,function(res){
			      ...
			      $("#laimi-self").attr('worder_id',res.worder_id);
		...
		//询问框
		$(document).on("click","#laimi-self", function() {
			var id = $(this).attr('worder_id');
			layer.confirm('确定自取吗？', {
		  	btn: ['确定自取','取消'] //按钮
		  },function(){
		  	$.post('wechat_shop_order_self_do.php', {worder_id:id}, function(res){
			  	if(res == 0 || res == 3){
			  		window.location.reload();
			  	}else{
			  		objlayer.alert('自取失败，请联系管理员', {
			  			title: '提示信息'
			  		});
			  	}
			  })
