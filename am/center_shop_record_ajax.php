...
function record_list(){
	...
	$arr = array();
	$strsql = "SELECT count(card_record_id) as mycount FROM " . $GLOBALS['gdb']->fun_table2('card_record') . " where card_id = ".$GLOBALS['intid'];
	...
	foreach($arrrecord as &$row){
		...
		if($row['card_record_type'] == 2){
			$strsql = "SELECT sum(c_mcombo_price * card_record2_mcombo_ccount) AS allprice,sum(c_mcombo_rprice * card_record2_mcombo_ccount) AS allrprice FROM " .$GLOBALS['gdb']->fun_table2('card_record2_mcombo'). " where card_record2_mcombo_type=1 and card_record_id=".$row['card_record_id'];
			$hresult = $GLOBALS['gdb']->fun_query($strsql);
			$arr = $GLOBALS['gdb']->fun_fetch_assoc($hresult);
			if(!empty($arr)){
				$row['mcombo_money'] = $arr['allprice'];
				$row['mcombo_money2'] = $arr['allrprice'];
			}
