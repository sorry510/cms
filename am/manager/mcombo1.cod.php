...
<?php echo $this->fun_fetch('inc_left', ''); ?>
		<div id="laimi-content" class="layui-body">
			<div class="layui-tab layui-tab-brief">
				...
				<div id="laimi-main" class="p-mcombo1 layui-tab-content">
<form class="layui-form">
	<div class="laimi-tools layui-form-item">		
		...
		<div class="laimi-float-right">
			<a href="mcombo1_add.php" class="layui-btn">新增计次套餐</a>
		</div>
	</div>
</form>
<table class="layui-table">
	...
	<tbody>
		...
		<tr>
			...
			<td><?php echo $row['mylimit']; ?></td>
			<td><?php echo $row['myact']; ?></td>
			<td><?php echo $row['mystate']; ?></td>
			<td>
				<button class="layui-btn layui-btn-mini" onclick="window.location='mcombo1_edit.php?id=<?php echo $row['mcombo_id']; ?>'">
					<svg class="laimi-bicon" aria-hidden="true"><use xlink:href="#icon-bianji"></use></svg>
					修改
				</button>
				<?php if($row['mcombo_state'] == 1) { ?>
				<button class="layui-btn layui-btn-mini layui-bg-red laimi-state1" value="<?php echo $row['mcombo_id']; ?>">
					<svg class="laimi-bicon" aria-hidden="true"><use xlink:href="#icon-tingyong"></use></svg>
					停用
				</button>
				<?php } else { ?>
				<button class="layui-btn layui-btn-mini layui-bg-blue laimi-state2" value="<?php echo $row['mcombo_id']; ?>">
					<svg class="laimi-bicon" aria-hidden="true"><use xlink:href="#icon-dui"></use></svg>
					启用
				</button>
				<?php } ?>
				<button class="layui-btn layui-btn-mini layui-btn-primary laimi-del" value="<?php echo $row['mcombo_id']; ?>">
	...
	layui.use(["layer", "element", "laypage", "form"], function() {
		...
		objpage.render({
			...
			jump: function(obj, first) {
				if(!first) {
					window.location = "mcombo1.php?<?php echo api_value_query($this->_data['request']);?>&page=" + obj.curr;
				}
			}
		});
			...
			objlayer.open({
			    ...
			    $.getJSON(url,{id:id},function(res){
			      ...
			      for (var i = 0; i < res.mgoods.length; i++){
			      	$("#laimi-mgoods").append('<tr><td>'+res.mgoods[i].mgoods_name+'</td><td>￥'+res.mgoods[i].mgoods_price+'</td><td>'+res.mgoods[i].mcombo_goods_count+'</td></tr>')
			      }
			    });
				}
			});
		...
		$(".laimi-del").on("click", function() {
			var strid = $(this).val();
			objlayer.confirm('确定要删除该套餐吗？', {icon:0, title:'提示', shadeClose:true}, function(hindex) {
			  $.post('mcombo1_delete_do.php', {id:strid}, function(strdata) {
			  	if(strdata == 0) {
			  		window.location.reload();
			  	} else {
			  		objlayer.alert('删除套餐失败，请联系管理员！', {
			  			title: '提示信息'
			  		});
			  	}
			  });
		...
		$(".laimi-state1").on("click", function() {
			var strid = $(this).val();
		  $.post('mcombo1_state_do.php', {id:strid,state:2}, function(strdata) {
        if(strdata == 0) {
          window.location.reload();
        } else {
          objlayer.alert('停用套餐失败，请联系管理员！', {
		  			title: '提示信息'
		  		});
        }
      });
		...
		$(".laimi-state2").on("click", function() {
			var strid = $(this).val();
		  $.post('mcombo1_state_do.php', {id:strid,state:1}, function(strdata) {
        if(strdata == 0) {
          window.location.reload();
        } else {
          objlayer.alert('启用套餐失败，请联系管理员！', {
		  			title: '提示信息'
		  		});
        }
      });
