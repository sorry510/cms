...
<table class="layui-table">
	...
	<tbody>
		...
		<tr>
			...
			<td>
				<button class="layui-btn layui-btn-mini" onclick="window.location='mcombo2_edit.php?id=<?php echo $row['mcombo_id']; ?>'">
	...
	layui.use(["layer", "element", "laypage", "form"], function() {
		...
		$(".laimi-del").on("click", function() {
			...
			objlayer.confirm('确定要删除该套餐吗？', {icon:0, title:'提示', shadeClose:true}, function(hindex) {
			  $.post('mcombo2_delete_do.php', {id:strid}, function(strdata) {
			  	if(strdata == 0) {
		...
		$(".laimi-state1").on("click", function() {
			var strid = $(this).val();
		  $.post('mcombo2_state_do.php', {id:strid,state:2}, function(strdata) {
        if(strdata == 0) {
          window.location.reload();
        } else {
		...
		$(".laimi-state2").on("click", function() {
			var strid = $(this).val();
		  $.post('mcombo2_state_do.php', {id:strid,state:1}, function(strdata) {
        if(strdata == 0) {
          window.location.reload();
        } else {
