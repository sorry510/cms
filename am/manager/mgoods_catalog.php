...
function get_mgoods_catalog_list() {
	...
	$arr = array();
	$strsql = "SELECT COUNT(mgoods_catalog_id) AS mycount FROM " . $GLOBALS['gdb']->fun_table2('mgoods_catalog');
	...
	. " ORDER BY mgoods_catalog_id DESC LIMIT " . $intoffset . ", " . $intpagesize;
	$hresult = $GLOBALS['gdb']->fun_query($strsql);
