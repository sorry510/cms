...
function get_sgoods_list() {
	...
	$strsql = "SELECT a.*, b.sgoods_catalog_id, b.sgoods_catalog_name, c.shop_name FROM "
	. "(SELECT sgoods_id, sgoods_catalog_id, shop_id, sgoods_type, sgoods_code, sgoods_name, sgoods_jianpin, sgoods_price, sgoods_cprice, sgoods_state FROM "
	. $GLOBALS['gdb']->fun_table2('sgoods') . " WHERE 1 = 1" . $strwhere . " ORDER BY sgoods_id DESC LIMIT " . $intoffset . ", " . $intpagesize
	. ") AS a LEFT JOIN " . $GLOBALS['gdb']->fun_table2('sgoods_catalog') . " AS b ON a.sgoods_catalog_id = b.sgoods_catalog_id "
	. "LEFT JOIN (SELECT shop_id, shop_name FROM " . $GLOBALS['gdb']->fun_table('shop') . " WHERE company_id = " . api_value_int0($GLOBALS['_SESSION']['login_cid'])
	. ") AS c ON a.shop_id = c.shop_id ORDER BY a.sgoods_id DESC";
	$hresult = $GLOBALS['gdb']->fun_query($strsql);
