...
function get_store_info_list() {
	...
	$strwhere = '';
	$strwhere .= " AND a.mgoods_id <> 0 AND a.shop_id IN (SELECT shop_id FROM " . $GLOBALS['gdb']->fun_table('shop') . " WHERE company_id = "
	...
	$strsql = "SELECT a.store_info_id, a.store_info_count, b.mgoods_code, b.mgoods_name, b.mgoods_price, b.mgoods_cprice, c.mgoods_catalog_name, d.shop_name FROM "
	. $GLOBALS['gdb']->fun_table2('store_info') . " AS a LEFT JOIN " . $GLOBALS['gdb']->fun_table2('mgoods') . " AS b ON a.mgoods_id = b.mgoods_id LEFT JOIN "
	. $GLOBALS['gdb']->fun_table2('mgoods_catalog') . " AS c ON b.mgoods_catalog_id = c.mgoods_catalog_id LEFT JOIN "
