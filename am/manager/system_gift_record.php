...
$gtemplate->fun_show('system_gift_record');
...
function get_gift_record_list() {
	...
	$strsql = "SELECT a.*, b.shop_name FROM (SELECT gift_record_id, shop_id, gift_record_score, gift_record_atime, c_card_code, c_card_name FROM "
	. $GLOBALS['gdb']->fun_table2('gift_record') . " WHERE 1 = 1" . $strwhere . " ORDER BY gift_record_id DESC LIMIT " . $intoffset . ", " . $intpagesize
	. ") AS a LEFT JOIN " . $GLOBALS['gdb']->fun_table('shop') . " AS b ON a.shop_id = b.shop_id ORDER BY a.gift_record_id DESC";
	$hresult = $GLOBALS['gdb']->fun_query($strsql);
