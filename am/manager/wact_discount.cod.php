			...
			<div class="layui-tab layui-tab-brief">
				<ul class="layui-tab-title">
					...
					<li class="<?php if ($this->_data['request']['expire'] ==1) {
						...
						<a href="wact_discount.php?expire=1">已结束</a>
					</li>
				</ul>
...
<form class="layui-form">
	<div class="laimi-tools layui-form-item">
		...
		<div class="laimi-float-right">
			<a class="layui-btn" href="wact_discount_add.php">新增打折活动</a>
		</div>
	</div>
</form>
<table class="layui-table">
	...
	<tbody>
		<?php foreach($this->_data['wact_discount_list']['list'] as $row) { ?>
		<tr>
			<td><?php echo date('Y-m-d H:i',$row['wact_discount_atime']) ; ?></td>
			<td><?php echo $row['wact_discount_name']; ?></td>
			<td><?php echo date('Y-m-d',$row['wact_discount_start']); ?></td>
      <td><?php echo date('Y-m-d',$row['wact_discount_end']); ?></td>
			<td><?php echo $row['wact_discount_memo']; ?></td>
			<td<?php
        if ($row['wact_discount_state'] == 1) {
          if ($row['datstate'] == '活动中') {
          ...
          ;?></td>
			<td>
        <button onclick="window.location.href='wact_discount_edit.php?id=<?php echo $row['wact_discount_id'];?>'" <?php if ($row['datstate'] == '已结束') {
          echo "style='display:none'";
        } ;?> class="layui-btn layui-btn-mini laimi-edit" value="<?php echo $row['wact_discount_id']; ?>">
          <svg class="laimi-bicon" aria-hidden="true"><use xlink:href="#icon-bianji"></use></svg>
          修改
        </button>
        <button <?php if ($row['datstate'] == '已结束' || $row['datstate'] == '活动中') {
          echo "style='display:none'";
        } ;?> class="layui-btn layui-btn-primary layui-btn-mini laimi-del" value="<?php echo $row['wact_discount_id']; ?>">
          <svg class="laimi-hicon" aria-hidden="true"><use xlink:href="#icon-clear"></use></svg>
          删除
        </button>
        <button <?php if ($row['datstate'] == '已结束' || $row['datstate'] == '未开始') {
          echo "style='display:none'";
        } ;?> class="layui-btn layui-btn-mini <?php if ($row['wact_discount_state'] == 1) {
          echo " layui-bg-red laimi-stop";
        }else{echo " layui-bg-blue laimi-start";} ; ?> " value="<?php echo $row['wact_discount_id']; ?>">
          <svg class="laimi-bicon" aria-hidden="true"><use xlink:href="<?php if ($row['wact_discount_state'] == 1) {
          	echo '#icon-tingyong';
          }else{
          	echo '#icon-dui';		
          };?>"></use></svg>
          <?php if ($row['wact_discount_state'] == 1) {
            echo "停止";
          }else{ echo "启用";} ;?>
        </button>
      </td>
...
<?php echo $this->fun_fetch('inc_foot', ''); ?>
	...
	layui.use(["element", "laydate", "laypage", "layer"], function() {
		...
		objpage.render({
			...
			count: <?php echo $this->_data['wact_discount_list']['allcount'];?>,
			limit: 50,
			curr: <?php echo $this->_data['wact_discount_list']['pagenow'];?>,
	  ...
	  $(".laimi-stop").on("click", function() {
			var url="wact_discount_stop_do.php";
      var data = {
        id:$(this).val(),
        type:'stop'
      }
	  ...
	  $(".laimi-del").on("click", function() {
			...
			objlayer.confirm('你确定要删除吗', {icon: 0, title:'提示',shadeClose: true}, function(index){
			  $.post('wact_discount_delete_do.php', {id:id}, function(res){
			  	if(res == 0){
			  		window.location.reload();
			  	}else{
			  		objlayer.alert('删除失败，请联系管理员', {
