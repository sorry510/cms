...
if ($intreturn == 0) {
	...
	if($arr['wact_discount_end'] < $inttime){
		$intreturn = 101;
	}else{
		$intreturn = 0;
	}
...
if($intreturn == 0) {
	$strsql = "UPDATE " . $gdb->fun_table2('wact_discount') . " SET  wact_discount_name = '" . $sqlname . "', wact_discount_start = "
	. $intstart . ", wact_discount_end = " . $intend . ", wact_discount_memo = '" . $sqlmemo . "', wact_discount_ctime =". $inttime ." WHERE wact_discount_id = " . $intid . " LIMIT 1";
	$hresult = $gdb->fun_do($strsql);
	if($hresult == FALSE) {
		$intreturn = 6;
	}
