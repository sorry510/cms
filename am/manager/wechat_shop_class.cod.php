...
<table class="layui-table">
	...
	<tbody>
		<?php foreach($this->_data['wgoods_catalog_list']['list'] as $row) { ?>
		<tr>
			<td><?php echo $row['wgoods_catalog_name']; ?></td>
			<td>
				<button class="layui-btn layui-btn-mini laimi-edit" value="<?php echo $row['wgoods_catalog_id'];?>">
					<svg class="laimi-bicon" aria-hidden="true"><use xlink:href="#icon-bianji"></use></svg>
					修改
				</button>
				<button class="layui-btn layui-btn-primary layui-btn-mini laimi-del" value="<?php echo $row['wgoods_catalog_id'];?>">
			...
			<form class="layui-form">
			  <div class="layui-form-item">
			    ...
			    <div class="layui-input-block">
			      <input class="layui-input laimi-input-200" type="text" name="txtwgoods_catalog_name" lay-verify="required">
			    </div>
			  </div>		    	  
			  <div class="layui-form-item">
			    <div class="layui-input-block">
		...
		<div id="laimi-modal-edit" class="laimi-modal">
			<form class="layui-form">
			  <div class="layui-form-item">
			    ...
			    <div class="layui-input-block">
			      <input class="layui-input laimi-input-200" type="text" name="txtwgoods_catalog_name" lay-verify="required">
			      <input class="layui-input laimi-input-200" type="hidden" name="txtwgoods_catalog_id">
			    </div>
			  </div>		    	  
			  <div class="layui-form-item">
			    <div class="layui-input-block">
...
<?php echo $this->fun_fetch('inc_foot', ''); ?>
	...
	layui.use(["element", "laypage", "layer", "form"], function() {
		...
		objpage.render({
			...
			count: <?php echo $this->_data['wgoods_catalog_list']['allcount'];?>,
			limit: 50,
			curr: <?php echo $this->_data['wgoods_catalog_list']['pagenow'];?>,
		...
		$(".laimi-edit").on("click", function() {
			...
			var wgoods_catalog_id = $(this).val();
		  var wgoods_catalog_name = $(this).parent().siblings().eq(0).text();
		  console.log(wgoods_catalog_name);
		  $("#laimi-modal-edit input[name='txtwgoods_catalog_name']").val(wgoods_catalog_name);
		  $("#laimi-modal-edit input[name='txtwgoods_catalog_id']").val(wgoods_catalog_id);
		});
		//添加商品submit
		objform.on("submit(laimi-submitadd)", function(data) {
			var _self = $(this);
		  _self.attr('disabled',true);
		  var url="wechat_shop_class_add_do.php";
		  $.post(url,data.field,function(res){
		    if(res=='0'){
		      window.location.href='wechat_shop_class.php';
		    }else{
		...
		objform.on("submit(laimi-submitedit)", function(data) {
		  var _self = $(this);
		  _self.attr('disabled',true);
		  var url="wechat_shop_class_edit_do.php";
	  ...
	  $(".laimi-del").on("click", function() {
			...
			objlayer.confirm('你确定要删除吗', {icon: 0, title:'提示',shadeClose: true}, function(index){
			  $.post('wechat_shop_class_delete_do.php', {id:id}, function(res){
			  	if(res == 0){
