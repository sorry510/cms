...
function get_wgoods_list() {
	...
	$strsql = "SELECT a.*, b.wgoods_catalog_id, b.wgoods_catalog_name FROM ( SELECT wgoods_id, wgoods_catalog_id, wgoods_type, wgoods_code, wgoods_name, wgoods_price, wgoods_cprice, wgoods_act, wgoods_state , wgoods_reward,wgoods_store, wgoods_commend FROM " . $GLOBALS['gdb']->fun_table2('wgoods') . " where 1=1 ".$strwhere." ORDER BY wgoods_id DESC LIMIT ". $intoffset . ", " . $intpagesize . ") AS a left join " . $GLOBALS['gdb']->fun_table2('wgoods_catalog') . " AS b on a.wgoods_catalog_id = b.wgoods_catalog_id"; 
	...
	foreach($arr as $key => $row) {
		$arr[$key]['commend'] = '';
		if($row['wgoods_commend'] == 1) {
			$arr[$key]['commend'] = '推荐';
		} else if($row['wgoods_commend'] == 2) {
			$arr[$key]['commend'] = '不推荐';
		}
	}
