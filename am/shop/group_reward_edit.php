...
function get_group_reward_info() {
	...
	. $GLOBALS['gdb']->fun_table2('group_reward')
	. " WHERE worker_group_id = " . $GLOBALS['intid'] . " AND shop_id = " . api_value_int0($GLOBALS['_SESSION']['login_sid']) . " LIMIT 1";
...
function get_mgoods_list() {
	...
	. " WHERE worker_group_id = " . $GLOBALS['intid'] . " AND shop_id = " . api_value_int0($GLOBALS['_SESSION']['login_sid']) . " AND mgoods_id > 0) AS b "
	. "ON a.mgoods_id = b.mgoods_id ORDER BY a.mgoods_id DESC";
	$hresult = $GLOBALS['gdb']->fun_query($strsql);
	$arr = $GLOBALS['gdb']->fun_fetch_all($hresult);
	return $arr;
}
...
function get_sgoods_list() {
	$arr = array();
	$strsql = "SELECT a.*, b.*, a.sgoods_id AS goods_id FROM (SELECT sgoods_id, sgoods_catalog_id, sgoods_name, sgoods_price FROM " . $GLOBALS['gdb']->fun_table2('sgoods')
	. " WHERE shop_id = " . api_value_int0($GLOBALS['_SESSION']['login_sid']) . " AND sgoods_state = 1) AS a "
	. "LEFT JOIN (SELECT group_reward2_id, sgoods_id, group_reward2_money, group_reward2_percent FROM " . $GLOBALS['gdb']->fun_table2('group_reward2')
	. " WHERE worker_group_id = " . $GLOBALS['intid'] . " AND shop_id = " . api_value_int0($GLOBALS['_SESSION']['login_sid']) . " AND sgoods_id > 0) AS b "
	. "ON a.sgoods_id = b.sgoods_id ORDER BY a.sgoods_id DESC";
	$hresult = $GLOBALS['gdb']->fun_query($strsql);
	$arr = $GLOBALS['gdb']->fun_fetch_all($hresult);
	return $arr;
}
...
function get_mcombo_list() {
	...
	. " WHERE worker_group_id = " . $GLOBALS['intid'] . " AND shop_id = " . api_value_int0($GLOBALS['_SESSION']['login_sid']) . " AND mcombo_id > 0) AS b "
	. "ON a.mcombo_id = b.mcombo_id ORDER BY a.mcombo_id DESC";
	$hresult = $GLOBALS['gdb']->fun_query($strsql);
	$arr = $GLOBALS['gdb']->fun_fetch_all($hresult);
	return $arr;
}
