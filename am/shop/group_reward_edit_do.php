...
if($intreturn == 0) {
	...
	if($deccreate > 0 || $decfill2 > 0 || $decfill3 > 0) {
		...
		. "group_reward_pguide, group_reward_atime, group_reward_ctime) VALUES (" . $intid . ", " . api_value_int0($GLOBALS['_SESSION']['login_sid'])
		. ", " . $deccreate . ", " . $decfill2 . ", " . $decfill3 . ", " . $decguide2 . ",  " . $decguide3 . ", " . $inttime . ", 0)";
		$gdb->fun_do($strsql);
	}
}
	...
	if(!empty($arr1)) {
		foreach ($arr1 as $row) {
			...
			if($decpercent > 0 || $decmoney > 0) {
				...
				. ", " . $intcatalog . ", ". $intmgoods . ", 0, 0, 0, " . $decmoney . " , ". $decpercent . ", " . $inttime . ", 0)";
				$gdb->fun_do($strsql);
			}
		}
	}
...
if($intreturn == 0) {
	if(!empty($arr2)) {
		foreach ($arr2 as $row) {
			$intcatalog = api_value_int0($row['sgoods_catalog_id']);
			...
			if($decpercent > 0 || $decmoney > 0) {
				$strsql = "INSERT INTO " . $gdb->fun_table2('group_reward2') . " (worker_group_id, shop_id, mgoods_catalog_id, mgoods_id, sgoods_catalog_id, sgoods_id, mcombo_id, "
				. "group_reward2_money, group_reward2_percent, group_reward2_atime, group_reward2_ctime) VALUES (" . $intid . ", " . api_value_int0($GLOBALS['_SESSION']['login_sid'])
				. ", 0, 0, " . $intcatalog . ", ". $intsgoods . ", 0, " . $decmoney . " , ". $decpercent . ", " . $inttime . ", 0)";
				$gdb->fun_do($strsql);
			}
		}
	}
	...
	if(!empty($arr3)) {
		foreach ($arr3 as $row) {
			...
			if($decmoney > 0) {
				$strsql = "INSERT INTO " . $gdb->fun_table2('group_reward2') . " (worker_group_id, shop_id, mgoods_catalog_id, mgoods_id, sgoods_catalog_id, sgoods_id, mcombo_id, "
				. "group_reward2_money, group_reward2_percent, group_reward2_atime, group_reward2_ctime) VALUES (" . $intid . ", " . api_value_int0($GLOBALS['_SESSION']['login_sid'])
				. ", 0, 0, 0, 0, " . $intmcombo . ", " . $decmoney . " , ". $decpercent . ", " . $inttime . ", 0)";
				$gdb->fun_do($strsql);
			}
		}
	}
