...
$strsql = "SELECT a.*,b.shop_name FROM (SELECT card_record_id,card_record_code,card_id,shop_id,card_record_type,card_record_cmoney,card_record_hmoney,card_record_ymoney,card_record_mmoney,card_record_jmoney,card_record_smoney,card_record_emoney,card_record_pay,card_record_xianjin,card_record_yinhang,card_record_weixin,card_record_zhifubao,card_record_kakou,card_record_tuan,card_record_score,card_record_state,card_record_atime,c_card_type_name,c_card_type_discount,c_card_code,c_card_name,c_card_phone,c_card_sex,c_user_id,c_user_name FROM " . $gdb->fun_table2('card_record') . " where card_record_id = ".$intcard_record_id." limit 1) AS a LEFT JOIN ". $gdb->fun_table('shop') ." AS b on a.shop_id = b.shop_id";
...
if(!empty($arr)){
	...
	if($arr['card_record_type'] == 3){
		//消费商品
		$strsql = "SELECT mgoods_id,sgoods_id,card_record3_goods_count,c_mgoods_name,c_mgoods_price,c_mgoods_rprice,c_sgoods_name,c_sgoods_price,c_sgoods_rprice,c_worker_name FROM " .$gdb->fun_table2('card_record3_goods'). " where card_record_id=".$arr['card_record_id'];
		$hresult = $gdb->fun_query($strsql);
	...
	//导购员
	$strsql = "SELECT c_worker_name FROM " .$gdb->fun_table2('worker_reward'). " where worker_reward_type = 4 AND c_card_record_id = ".$arr['card_record_id'];
	$hresult = $gdb->fun_query($strsql);
	$arrguider = $gdb->fun_fetch_assoc($hresult);
	$arr['guider'] = empty($arrguider['c_worker_name']) ? '无' : $arrguider['c_worker_name'];
}
