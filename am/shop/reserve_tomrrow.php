...
function get_reserve_list() {
	...
	if (empty($GLOBALS['intfrom']) && empty($GLOBALS['intto']) && empty($GLOBALS['sqlsearch'])) {
		$strwhere3 = " AND reserve_dtime >=".($GLOBALS['inttoday']+86400)." AND reserve_dtime <".($GLOBALS['inttoday']+172800);
	}

	$arr = array();
	$strsql = 'SELECT count(reserve_id) as datcount FROM (SELECT reserve_id, card_id, shop_id, reserve_dtime FROM ' . $GLOBALS['gdb']->fun_table2('reserve') . ' WHERE shop_id = ' .api_value_int0($GLOBALS['_SESSION']['login_sid']). ') as a LEFT JOIN '. $GLOBALS['gdb']->fun_table2('card') .' as b ON a.card_id = b.card_id WHERE 1 = 1 ' . $strwhere . $strwhere2 . $strwhere3;
	...
	$strsql = 'SELECT a.reserve_id,a.card_id,a.reserve_name,a.reserve_phone,a.reserve_count,a.reserve_state,a.reserve_atime,a.reserve_dtime,a.reserve_here,a.reserve_type,a.reserve_memo,b.card_code FROM (SELECT reserve_id,card_id,reserve_name,reserve_here,reserve_phone,reserve_count,reserve_state,reserve_atime,reserve_dtime,reserve_type,reserve_memo FROM ' . $GLOBALS['gdb']->fun_table2('reserve') . ' WHERE shop_id = ' . api_value_int0($GLOBALS['_SESSION']['login_sid']) . $strwhere . $strwhere3 .' ORDER BY reserve_dtime LIMIT ' . $intoffset . ', ' . $intpagesize . ') as a LEFT JOIN (SELECT card_code,card_id FROM ' . $GLOBALS['gdb']->fun_table2('card') .' WHERE card_state =1 ) as b on a.card_id = b.card_id WHERE 1=1 ' . $strwhere2;
