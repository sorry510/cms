...
function get_store_list() {
	...
	if($intfrom > 0) {
		$strwhere = $strwhere . ' AND store_atime >= ' . $intfrom;
	}
	if($intto > 0) {
		$strwhere = $strwhere . ' AND store_atime < ' . $intto;
	}
	...
	$strsql = "SELECT store_id, store_type, store_money, store_operator, store_memo, store_state, store_time FROM " . $GLOBALS['gdb']->fun_table2('store')
	. " WHERE 1 = 1" . $strwhere . " ORDER BY store_id DESC LIMIT " . $intoffset . ", " . $intpagesize;
	$hresult = $GLOBALS['gdb']->fun_query($strsql);
	...
	foreach($arrlist as &$row) {
		...
		$row['mystate'] = '';
		if($row['store_state'] == 1) {
			$row['mystate'] = '未确认';
		} else if($row['store_state'] == 2) {
			$row['mystate'] = '已确认';
		}
		$row['mytime'] = '';
		if($row['store_time'] > 0) {
			$row['mytime'] = date('Y-m-d', $row['store_time']);
		}
	}
