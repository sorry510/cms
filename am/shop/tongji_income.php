...
function get_card_type() {
	...
	if ($GLOBALS['intdata_time'] == 1) {
		for($i = 0;$i < 30;$i++){
			...
			$strsql = "SELECT count(card_id) as card_num,card_type_name FROM (SELECT card_type_id,card_type_name FROM " . $GLOBALS['gdb']->fun_table2('card_type') . ") as a LEFT JOIN (SELECT card_id,card_type_id FROM ".$GLOBALS['gdb']->fun_table2('card')." WHERE card_state != 3 AND card_atime >". $intfrom ." AND card_atime <= " . $intto .  $GLOBALS['strwhere'].") as b on a.card_type_id = b.card_type_id GROUP BY a.card_type_id order by a.card_type_id";
			...
			foreach ($arr_card_type as $key => $value) {
				if ($i == 0) {
					...
					$arr[$key]['card_type_name'] = $arr_card_type[$key]['card_type_name'];
				}else{
