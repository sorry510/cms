...
if($intreturn == 0) {
	$str = api_value_https('https://api.weixin.qq.com/sns/oauth2/access_token?appid=' . $arrcompany_config_weixin['appid'] . '&secret=' . $arrcompany_config_weixin['appsecret']
...
$arrcard = array();
if($intreturn == 0) {
	$strsql = "SELECT card_id, card_okey FROM " . $gdb->fun_table2('card') . " WHERE card_okey = '" . $gdb->fun_escape($stropenid) . "' LIMIT 1";
	$hresult = $gdb->fun_query($strsql);
...
if($intreturn == 0) {
	if($strgoto == 'index') {
		...
		if($arrcompany_config_trade['wmp_module'] != 1) {
			...
			header('Location: reserve_shop.php');
		}
	}
} else {
	header('Location: error.php');
