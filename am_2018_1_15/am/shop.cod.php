  ...
  function pullupRefresh() {
			...
			if(res.list.length > 0){
				...
				mui.each(res.list, function(k, v){
					...
					html += '<a href="detail.php?id='+v.wgoods_id+'">'+
						        '<img class="mui-media-object" style="width:100%; height:auto;" src="'+src+'">'+
	...
	wx.ready(function() {
		wx.onMenuShareTimeline({
			title: "分享测试", // 分享标题
			link: "<?php echo $GLOBALS['gconfig']['project']['url_mobile'];?>/wx_share.php?goto=1&cid=<?php echo $GLOBALS['_SESSION']['login_cid']; ?>&uid=<?php echo $GLOBALS['_SESSION']['login_id']; ?>", // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
