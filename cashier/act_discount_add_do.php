...
if ($intreturn == 0) {
	if (empty($sqlname) || empty($strshop_id) || empty($intclient)) {
		$intreturn = 1;
	}
}
...
if($intreturn == 0) {
	$strsql = "INSERT INTO " . $gdb->fun_table2('act_discount') . " (act_discount_atime, act_discount_name, act_discount_client,  act_discount_start,  act_discount_end,  act_discount_state,  act_discount_memo,  act_discount_shop) VALUES ("
	.time() . ", '" . $sqlname . "', " . $intclient . ", ". $intstart .",".$intend.",".$intstate. ", '" .$sqlmemo. "',". $intshop .")";
...
if($intreturn == 0) {
	foreach ($strshop_id as $key => $value) {
		$strsql = "INSERT INTO " . $gdb->fun_table2('act_discount_shop') . " (act_discount_id,shop_id) VALUES (" .$id . " , " .$value . ")";
		$hresult = $gdb->fun_do($strsql);
		if($hresult == FALSE) {
			$intreturn = 7;
		}
