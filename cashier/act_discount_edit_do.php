...
if ($intreturn == 0) {
	if (empty($sqlname) || empty($strshop_id) || empty($intclient) || empty($intid)) {
		$intreturn = 1;
	}
}
...
if($intreturn == 0) {
	$strsql = "UPDATE " . $gdb->fun_table2('act_discount') . " SET  act_discount_name = '" . $sqlname . "', act_discount_shop = "
	. $intshop . ", act_discount_client = " . $intclient . ",  act_discount_start = "
	. $intstart . ", act_discount_end = " . $intend . ", act_discount_memo = '" . $sqlmemo ."', act_discount_state =". $intstate . ", act_discount_ctime =". $time ." WHERE act_discount_id = " . $intid . " LIMIT 1";
	$hresult = $gdb->fun_do($strsql);
	if($hresult == FALSE) {
		$intreturn = 6;
	}
...
if($intreturn == 0) {
	$strsql = "DELETE FROM " . $gdb->fun_table2('act_discount_shop') . " WHERE act_discount_id = " . $intid ;
	$hresult = $gdb->fun_do($strsql);
	if($hresult == FALSE) {
		$intreturn = 7;
	}
	foreach ($strshop_id as $key => $value) {
		$strsql = "INSERT INTO " . $gdb->fun_table2('act_discount_shop') . " (act_discount_id,shop_id) VALUES (" .$intid . " , " .$value . ")";
		$hresult = $gdb->fun_do($strsql);
		if($hresult == FALSE) {
			$intreturn = 8;
		}
...
if (!empty($arr2)) {
	if ($intreturn == 0) {
		foreach ($arr2 as $key => $value) {
			...
			$strsql = "INSERT INTO " . $gdb->fun_table2('act_discount_goods') . " (act_discount_id,mcombo_id,act_discount_goods_value,	act_discount_goods_price , c_mcombo_name , c_mcombo_price , act_discount_goods_atime) VALUES (" .$intid ." , ". $mcombo_id. " , " .$discount_goods_value ." , ". $discount_goods_price." , '" . $c_mcombo_name ."' , ". $c_mcombo_price . " , " . $time .")";
			$hresult = $gdb->fun_do($strsql);
			if($hresult == FALSE) {
				$intreturn = 12;
			}
