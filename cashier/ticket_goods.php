...
function get_ticket_goods_list() {
	...
	$strsql = 'SELECT ticket_goods_id,ticket_goods_begin,ticket_goods_name,ticket_goods_value,mgoods_name,ticket_goods_memo,ticket_goods_days,ticket_goods_atime FROM' . $GLOBALS['gdb']->fun_table2('ticket_goods') . 'as a LEFT JOIN '. $GLOBALS['gdb']->fun_table2('mgoods').' as b on a.mgoods_id = b.mgoods_id WHERE 1 = 1' . $strwhere .' ORDER BY ticket_goods_id DESC'.' LIMIT ' . $intoffset . ', ' . $intpagesize;
