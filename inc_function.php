...
function laimi_prefix2_value() {
	$strprefix2 = '';
	if(!empty($GLOBALS['_SESSION']['login_cid'])) {
...
function laimi_cart_count() {
	...
	if(!empty($GLOBALS['_SESSION']['login_cid'])) {
		$arr = array();
		$strsql = "SELECT count(wcart_id) AS mycount FROM " . $GLOBALS['gdb']->fun_table2('wcart') . " WHERE card_id = " . api_value_int0($GLOBALS['_SESSION']['login_id']);
		$hresult = $GLOBALS['gdb']->fun_query($strsql);
		$arr = $GLOBALS['gdb']->fun_fetch_assoc($hresult);
		if(!empty($arr)) {
			if(!empty($arr['mycount'])) {
				$intcount = $arr['mycount'];
			}
		}
	}
	return $intcount;
 ...
 *
 * @param int $wgoods_id
 * @param decimal $wgoods_price
 * @param decimal $wgoods_cprice
 * @return array [wgoods_iprice, wact_discount_id]
 */
function laimi_wgoods_price($wgoods_id = 0, $wgoods_price = 0, $wgoods_cprice = 0) {
	$arrreturn = array();
	$arrreturn['wgoods_iprice'] = 0;
	$arrreturn['wact_discount_id'] = 0;
	
	$inttime = time();
	$arr = array();
	$strsql = "SELECT wact_discount_goods_id, wact_discount_id, wact_discount_goods_price FROM " . $GLOBALS['gdb']->fun_table2('wact_discount_goods')
	. " WHERE wact_discount_id IN (SELECT wact_discount_id FROM " . $GLOBALS['gdb']->fun_table2('wact_discount')
	. " WHERE wact_discount_state = 1 AND wact_discount_start < " . $inttime . " AND wact_discount_end > " . $inttime . ")"
	. " AND wgoods_id = " . api_value_int0($wgoods_id) . " ORDER BY wact_discount_goods_price LIMIT 1";
	$hresult = $GLOBALS['gdb']->fun_query($strsql);
	$arr = $GLOBALS['gdb']->fun_fetch_assoc($hresult);
	if(empty($arr)) {
		if($wgoods_cprice > 0) {
			$arrreturn['wgoods_iprice'] = $wgoods_cprice;
		} else {
			$arrreturn['wgoods_iprice'] = $wgoods_price;
		}
	} else {
		if($wgoods_cprice > 0) {
			if($arr['wact_discount_goods_price'] < $wgoods_cprice) {
				$arrreturn['wgoods_iprice'] = $arr['wact_discount_goods_price'];
			} else {
				$arrreturn['wgoods_iprice'] = $wgoods_cprice;
			}
		} else {
			if($arr['wact_discount_goods_price'] < $wgoods_price) {
				$arrreturn['wgoods_iprice'] = $arr['wact_discount_goods_price'];
			} else {
				$arrreturn['wgoods_iprice'] = $wgoods_price;
			}
		}
		$arrreturn['wact_discount_id'] = $arr['wact_discount_id'];
	}
...
function laimi_pay_return($postarr){
    ...
    if(array_key_exists("parentid", $extra))
    ...
    if(array_key_exists("parentid", $postarr))
	...
	if($pay_channel=='ALI_WEB'){
		...
		$app_id = $postarr['appid'];
		$out_trade_no = $postarr['out_trade_no'];
		$total_fee = round($postarr['total_fee'] / 100, 2);//元,微信付款真实价格
	    ...
	    $arrcart = $GLOBALS['gdb']->fun_fetch_all($hresult);
	      ...
	      $intworder_from = 0;
				...
				if($intpay == 31){
					...
					$strsql = "UPDATE ".$GLOBALS['gdb']->fun_table2('card')." SET s_card_ymoney=".$card_ymoney.",	s_card_smoney=s_card_smoney+".$total_fee.",card_ctime=".$inttime.",card_ltime=".$inttime." where card_id=".$arrcard['card_id']." limit 1";
		  	...
		  	if(!empty($arrparent)){
				...
				$intworder_id = $GLOBALS['gdb']->fun_insert_id();
				// echo "<pre>";
				// var_dump($arrcart);
				// echo "</pre>";
				// exit;
