...
if($intreturn == 0) {
	...
	if(!empty($arr)) {
		$gdb->pubprefix2 = $arr['system_code'] . "_" . str_pad($arr['company_id'], 4, '0', STR_PAD_LEFT) . "_";
	} else {
		$intreturn = 2;
	}
}
...
if($intreturn == 0) {
	if($arr3['user_type'] == 1) {
		echo $arr['system_code'] . '/manager';
	} else if($arr3['user_type'] == 2) {
		echo $arr['system_code'] . '/shop';
	} else if($arr3['user_type'] == 3) {
		echo $arr['system_code'] . '/cashier';
	}
} else {
	echo $intreturn;
}
