...
function get_sms_list() {
	...
	$arr = array();
	$strsql = 'SELECT count(act_ticket_id) as datcount FROM ' . $GLOBALS['gdb']->fun_table2('act_ticket') . ' WHERE act_ticket_atype = 2' . $strwhere;
	...
	,act_ticket_atime,act_ticket_info FROM' . $GLOBALS['gdb']->fun_table2('act_ticket') . ' WHERE act_ticket_atype = 2' . $strwhere .' ORDER BY act_ticket_id DESC'.' LIMIT ' . $intoffset . ', ' . $intpagesize;
