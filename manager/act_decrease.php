...
function get_act_decrease_list() {
	...
	if ($GLOBALS['intexpire'] == 1) {
		$strwhere = $strwhere . " AND " .$GLOBALS['inttime']." > act_decrease_end ";
	}else{
		$strwhere = $strwhere . " AND " .$GLOBALS['inttime']." <= act_decrease_end ";
	}
	...
	$strsql = 'SELECT act_decrease_id,act_decrease_name,act_decrease_client,act_decrease_man,act_decrease_jian,act_decrease_start,act_decrease_end,act_decrease_state,act_decrease_memo,act_decrease_atime FROM' . $GLOBALS['gdb']->fun_table2('act_decrease') . ' WHERE 1 = 1' . $strwhere .' ORDER BY act_decrease_id DESC'.' LIMIT ' . $intoffset . ', ' . $intpagesize;
	...
	foreach($arr as $key => $row) {
		$arr[$key]['datstate'] = '';
		if($row['act_decrease_end'] < $GLOBALS['inttime']) {
			$arr[$key]['datstate'] = '已结束';
		}elseif($row['act_decrease_start'] < $GLOBALS['inttime'] && $GLOBALS['inttime'] < $row['act_decrease_end']) {
			$arr[$key]['datstate'] = '活动中';
		}elseif ($GLOBALS['inttime'] < $row['act_decrease_start']) {
