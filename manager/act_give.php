...
function get_act_give_list() {
	...
	if ($GLOBALS['intexpire'] == 1) {
		$strwhere = $strwhere . " AND " .$GLOBALS['inttime']." > act_give_end ";
	}else{
		$strwhere = $strwhere . " AND " .$GLOBALS['inttime']." <= act_give_end ";
	}
	...
	foreach($arr as $key => $row) {
		$arr[$key]['datstate'] = '';
		if($row['act_give_end'] < $GLOBALS['inttime']) {
			$arr[$key]['datstate'] = '已结束';
		}elseif($row['act_give_start'] < $GLOBALS['inttime'] && $GLOBALS['inttime'] < $row['act_give_end']) {
			$arr[$key]['datstate'] = '活动中';
		}elseif ($GLOBALS['inttime'] < $row['act_give_start']) {
