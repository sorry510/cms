    ...
    <div class="am-modal-bd umain1">
      <form class="am-form am-form-horizontal" id="form2" method="get" action="mcombo_time_add_do.php">
        <div class="am-form-group">
          <label class="umodal-label am-form-label" for=""><span class="gtext-orange">*</span>套餐名称：</label>
          <div class="umodal-normal">
            <input type="text" name="mcombo_name" id="cgoodsname2" class="am-form-field uinput uinput-max cvalid" onKeyUp="query2()" required>
          </div>
          <div class="umodal-text" style="text-indent:2em;">简拼：</div>
          <div class="umodal-valid">
...
$('.cmodelopen').on('click', function(e) {
  // 验证变红
  $('#umcombo_timem1 .cvalid').each(function(){
    if($(this).val()==''){
      $(this).addClass('am-field-error');
    }
  })
  // 验证返回
  if($('#umcombo_timem1 .cvalid').hasClass("am-field-error")){
    return false;
  }
...
$('.cmodelopen3').on('click', function(e) {
  ...
  // 验证返回
  if($('#umcombo_timem2 .cvalid').hasClass("am-field-error")){
    return false;
  }
