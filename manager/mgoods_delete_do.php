...
$strmgoods_id = api_value_post('id');
...
//满送活动中的优惠券商品
$strsql = "SELECT act_give_id FROM ".$GLOBALS['gdb']->fun_table2('act_give'). " where c_mgoods_id = ".$intmgoods_id ." AND act_give_end > " time();
$hresult = $GLOBALS['gdb']->fun_query($strsql);
...
// 删除库存,对应各个店铺的mgoods
if($intreturn == 0){
	$strsql = "DELETE FROM " . $gdb->fun_table2('act_discount_goods') . " WHERE mgoods_id = " . $intmgoods_id;
	$hresult = $gdb->fun_do($strsql);
	if($hresult == FALSE) {
		$intreturn = 7;
	}
