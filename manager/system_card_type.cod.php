		...
		<div id="laimi-modal-add" class="laimi-modal">
			<form class="layui-form">
			  ...
			  <div class="layui-form-item">
			    ...
			    <div class="layui-input-inline">
			      <input class="layui-input laimi-input-200" type="text" name="txtdiscount" lay-verify="carddiscount|number" value="10">
			    </div>
			  ...
			  <div class="layui-form-item">
			    <div class="layui-input-block">
			      <button class="layui-btn laimi-button-100" lay-filter="laimi-add" lay-submit>完成</button>
	...
	<script type="text/html" id="laimi-edit">
		<div id="laimi-modal-edit" class="laimi-modal">
			<form class="layui-form">
			  ...
			  <div class="layui-form-item">
			    <div class="layui-input-block">
			      <button class="layui-btn laimi-button-100" lay-filter="laimi-edit" lay-submit>完成</button>
			      <button class="layui-btn layui-btn-primary" type="reset">重置</button>
			    </div>
			  </div>
			</form>
...
<?php echo $this->fun_fetch('inc_foot', ''); ?>
	...
	layui.use(["element", "layer", "form", "laytpl"], function() {
		...
		objform.on("submit(laimi-add)", function(data) {
			$.post('system_card_type_add_do.php', data.field, function(res){
				if(res == 0){
		...
		$(".laimi-edit").on("click", function() {
			$.getJSON('system_card_type_edit_ajax.php', {id:$(this).val()}, function(res){
		...
		objform.on("submit(laimi-edit)", function(data) {
			$.post('system_card_type_edit_do.php', data.field, function(res){
				if(res == 0){
					window.location.reload();
				}else{
		...
		$(".laimi-del").on("click", function() {
			...
			objlayer.confirm('你确定要删除吗', {icon: 0, title:'提示', shadeClose: true}, function(index){
			  $.post('system_card_type_delete_do.php', {id:id}, function(res){
			  	if(res == 0){
			  		window.location.reload();
			  	}else{
