...
if(empty($sqlphone)){
    $strsql = "SELECT company_phone FROM ".$gdb->fun_table('company')." WHERE company_id=".$GLOBALS['_SESSION']['login_cid'];
    $hresult = $gdb->fun_query($strsql);
    $arr = $gdb->fun_fetch_assoc($hresult);
    if(!empty($arr)){
        $sqlphone = $arr['company_phone'];
    }else{
        $intreturn = 1;
    }
}
$arrcompanyconfig = companyConfig();
$intsms_ycount = $arrcompanyconfig['sms_ycount'];
if($intsms_ycount < 1){
    $intreturn = 2;
}
...
if($intreturn == 0){
    ...
    if($response->Message == 'OK'){
        $strsql = "UPDATE ".$GLOBALS['gdb']->fun_table('company'). " SET company_sms_ycount=company_sms_ycount-1 WHERE company_id=".$GLOBALS['_SESSION']['login_cid'];
