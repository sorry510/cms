...
function get_card() {
	...
	$strsql = "SELECT count(card_id) as card_normal_num FROM " . $GLOBALS['gdb']->fun_table2('card') . " WHERE card_state = 1 AND card_edate >". $GLOBALS['inttime'] . $strwhere;
	$hresult = $GLOBALS['gdb']->fun_query($strsql);
	...
	$strsql = "SELECT count(card_id) as card_expired_num FROM " . $GLOBALS['gdb']->fun_table2('card') . " WHERE card_state = 1 AND card_edate < " . $GLOBALS['inttime'] . $strwhere;
	$hresult = $GLOBALS['gdb']->fun_query($strsql);
