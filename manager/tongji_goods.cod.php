		...
		<div class="layui-input-inline">
			<select name="goods_id">
				...
				<optgroup label="多店通用商品">
					...
					<option value="<?php echo '1,'.$row['mgoods_id'];?>"><?php echo $row['mgoods_name'];?></option>
					<?php };?>
				</optgroup>
				<optgroup label="单店商品">
					<?php foreach($this->_data['sgoods'] as $row) { ?>
					<option value="<?php echo '2,'.$row['sgoods_id'];?>"><?php echo $row['sgoods_name'];?></option>
					<?php };?>
				</optgroup>
			</select>
		</div>
