...
$strsql = "SELECT group_reward_id FROM ".$gdb->fun_table2('group_reward')." WHERE worker_group_id=".$intworker_group_id." and shop_id = 0 limit 1";
...
if($intreturn == 0){
	// 更新时删除之前所有商品提成记录
	if($group_reward_id != 0){
		$strsql = "DELETE FROM ".$gdb->fun_table2('group_reward2')." WHERE worker_group_id=".$intworker_group_id." and shop_id=0";
	...
	if(!empty($arrgoods)){
		foreach($arrgoods as $v){
			...
			if(array_key_exists("mgoods_id", $v)){
				$intmgoods_id = api_value_int0($v['mgoods_id']);
				$intmgoods_catalog_id = api_value_int0($v['mgoods_catalog_id']);
			}
			...
			if($intmgoods_id != 0){
				$strsql = "INSERT INTO ".$gdb->fun_table2('group_reward2')." (worker_group_id,shop_id,mgoods_catalog_id,mgoods_id,group_reward2_money,group_reward2_percent,group_reward2_atime,group_reward2_ctime) VALUES (".$intworker_group_id.",0,".$intmgoods_catalog_id.",".$intmgoods_id.",".$decmoney.",".$decpercent.",".$intnow.",".$intnow.")";
				// echo $strsql;
			}
			if($intmcombo_id != 0){
				$strsql = "INSERT INTO ".$gdb->fun_table2('group_reward2')." (worker_group_id,shop_id,mcombo_id,group_reward2_money,group_reward2_percent,group_reward2_atime,group_reward2_ctime) VALUES (".$intworker_group_id.",0,".$intmcombo_id.",".$decmoney.",".$decpercent.",".$intnow.",".$intnow.")";
			}
			$hresult = $GLOBALS['gdb']->fun_do($strsql);
			if($hresult == false){
				$intreturn = 4;
