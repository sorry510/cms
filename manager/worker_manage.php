...
function get_worker_list() {
	...
	$strsql = "SELECT a.*, b.shop_name, c.worker_group_name FROM (SELECT shop_id, worker_id, worker_group_id, worker_name, worker_code, worker_sex, worker_birthday, worker_phone, worker_education, worker_join, worker_wage, worker_config_guide, worker_config_reserve FROM " . $GLOBALS['gdb']->fun_table2('worker') . " where 1=1 ".$strwhere." ORDER BY worker_id DESC LIMIT ". $intoffset . ", " . $intpagesize . ") AS a ," . $GLOBALS['gdb']->fun_table('shop') . " AS b," . $GLOBALS['gdb']->fun_table2('worker_group') . " AS c WHERE a.shop_id = b.shop_id AND a.worker_group_id = c.worker_group_id ";
	$hresult = $GLOBALS['gdb']->fun_query($strsql);
