...
function record_info(){
	...
	foreach($arr as &$row)){
		...
		if($row['card_record_type'] == 3){
			...
			$hresult = $GLOBALS['gdb']->fun_query($strsql);
			$rowlist = $GLOBALS['gdb']->fun_fetch_all($hresult);
			$row['goods_list'] = $rowlist;
			$row['goods_count'] = 0;
			$row['goods_money'] = 0;//原价
			$row['goods_money2'] = 0;//真实价格
			foreach($rowlist as $row){
				$row['goods_count'] += $row['card_record3_goods_count'];
				$row['goods_money'] += ($row['c_mgoods_price'] + $row['c_sgoods_price']) * $row['card_record3_goods_count'];
				$row['goods_money2'] += ($row['c_mgoods_rprice'] + $row['c_sgoods_rprice']) * $row['card_record3_goods_count'];
			}
			...
			$hresult = $GLOBALS['gdb']->fun_query($strsql);
			$rowlist2 = $GLOBALS['gdb']->fun_fetch_all($hresult);
			$row['mcombo_goods_list2'] = $rowlist2;
			$row['mcombo_goods_count2'] = 0;
			$row['mcombo_goods_money2'] = 0;
			foreach($rowlist2 as $row){
				$row['mcombo_goods_count2'] += $row['card_record3_mgoods_count'];
				$row['mcombo_goods_money2'] += $row['c_mgoods_price'] * $row['card_record3_mgoods_count'];
			}
		}
		//买套餐
		if($row['card_record_type'] == 2){
			...
			$hresult = $GLOBALS['gdb']->fun_query($strsql);
			$rowlist = $GLOBALS['gdb']->fun_fetch_all($hresult);
			$row['mcombo_goods_list'] = $rowlist;
			$row['mcombo_goods_count'] = 0;
			$row['mcombo_goods_money'] = 0;
			foreach($rowlist as $row){
				$row['mcombo_goods_count'] += $row['card_record2_mcombo_gcount'];
				$row['mcombo_goods_money'] += $row['c_mgoods_price'] * $row['card_record2_mcombo_gcount'];
			}
		}
		//赠送什么券
		$rowticket = array();
		...
		$hresult = $GLOBALS['gdb']->fun_query($strsql);
		$rowticket = $GLOBALS['gdb']->fun_fetch_all($hresult);
		$row['ticket_list'] = $rowticket;
	}
	return $arr;
}