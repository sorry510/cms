...
$strsql = "SELECT valid_id FROM ".$GLOBALS['gdb']->fun_table2('valid')." WHERE valid_phone='".$sqlphone."' and valid_time>".(time()-60);
...
if($intreturn == 0){
  ...
  if($intreturn == 0){
      ...
      if($response->Message == 'OK'){
          ...
          $strsql = "INSERT INTO ".$GLOBALS['gdb']->fun_table2('valid')." (valid_ip,valid_verify,valid_phone,valid_time) VALUES ('".api_http_ip()."','".$rand."','".$sqlphone."',".time().")";
          $hresult = $gdb->fun_do($strsql);
      }else{
          $intreturn = 10;
      }
