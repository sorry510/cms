...
function get_card_list() {
	...
	$arr = array();
	$strsql = 'SELECT count(card_id) as datcount FROM ' . $GLOBALS['gdb']->fun_table2('card') . ' WHERE card_state = 1 AND shop_id = '. api_value_int0($GLOBALS['_SESSION']['login_sid']) . $strwhere;
	...
	$strsql = 'SELECT card_id,card_name,card_code,card_sex,card_atime,card_ltime,card_phone,card_birthday_date,card_edate,c_card_type_name FROM' . $GLOBALS['gdb']->fun_table2('card') . ' WHERE card_state = 1 AND shop_id = '.api_value_int0($GLOBALS['_SESSION']['login_sid']) . $strwhere .' ORDER BY card_id DESC'.' LIMIT ' . $intoffset . ' , ' . $intpagesize;
