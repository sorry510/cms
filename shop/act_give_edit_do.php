...
if ($intreturn == 0) {
	if (empty($sqlname) || empty($decman) || empty($intttype) || empty($strshop_id) || empty($intid)) {
		$intreturn = 1;
	}
}
...
if($intreturn == 0) {
	$strsql = "UPDATE " . $gdb->fun_table2('act_give') . " SET  act_give_name = '" . $sqlname . "', act_give_shop = "
	. $intshop . ", act_give_client = " . $intclient . ", act_give_man = " . $decman . ", act_give_ttype = " . $intttype .",  act_give_start = "
	. $intstart . ", act_give_end = " . $intend .", ticket_money_id = " . $intticket_money_id .", ticket_goods_id = " . $intticket_goods_id . ", act_give_memo = '" . $sqlmemo ."', act_give_state =". $intstate . ", c_ticket_name = '". $sqlc_ticket_name ."' , c_ticket_value = ". $decc_ticket_value ." , c_ticket_limit = ". $decc_ticket_limit." , c_ticket_days = ". $intc_ticket_days." , c_ticket_begin = ". $intc_ticket_begin ." , c_mgoods_id = ". $intc_mgoods_id." , c_mgoods_name = '". $sqlc_mgoods_name ."', act_give_ctime =". $time ." WHERE act_give_id = " . $intid . " LIMIT 1"; 
	$hresult = $gdb->fun_do($strsql);
	if($hresult == FALSE) {
		$intreturn = 8;
	}
...
if($intreturn == 0) {
	...
	foreach ($strshop_id as $key => $value) {
		$strsql = "INSERT INTO " . $gdb->fun_table2('act_give_shop') . " (act_give_id,shop_id) VALUES (" .$intid . " , " .$value . ")";
		$hresult = $gdb->fun_do($strsql);
		if($hresult == FALSE) {
			$intreturn = 10;
		}
