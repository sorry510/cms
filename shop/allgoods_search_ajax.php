...
switch($strflag){
	case 'all':
		...
		foreach($arr as $k=>$v){
			$strsql = "SELECT mgoods_id, mgoods_name, mgoods_price,mgoods_cprice FROM " . $GLOBALS['gdb']->fun_table2('mgoods')." WHERE 1=1 ". $strwheremgoods . " AND mgoods_catalog_id = ". $v['mgoods_catalog_id'] ." ORDER BY mgoods_id";
			$hresult = $GLOBALS['gdb']->fun_query($strsql);
		...
		foreach($arr2 as $k=>$v){
			$strsql = "SELECT sgoods_id, sgoods_name, sgoods_price,sgoods_cprice FROM " . $GLOBALS['gdb']->fun_table2('sgoods')." WHERE 1=1 ". $strwheresgoods . " AND sgoods_catalog_id = ". $v['sgoods_catalog_id'] ." ORDER BY sgoods_id";
			$hresult = $GLOBALS['gdb']->fun_query($strsql);
	...
	case 's':
		...
		foreach($arr as $k=>$v){
			$strsql = "SELECT sgoods_id, sgoods_name, sgoods_price,sgoods_cprice FROM " . $GLOBALS['gdb']->fun_table2('sgoods')." WHERE 1=1 ". $strwheremgoods . " AND sgoods_catalog_id = ". $v['sgoods_catalog_id'] ." ORDER BY sgoods_id";
			$hresult = $GLOBALS['gdb']->fun_query($strsql);
			$arrsgoods = $GLOBALS['gdb']->fun_fetch_all($hresult);
