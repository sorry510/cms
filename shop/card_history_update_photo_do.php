...
$strplace = '';
$intkey = 0;
$strsql = "SELECT card_history_id,card_history_photo1,card_history_photo2,card_history_photo3,card_history_photo4,card_history_photo5 FROM ".$gdb->fun_table2('card_history')." WHERE card_history_id=".$intid." and shop_id=".$intshop;
...
if(empty($arr)){
	...
	if($arr['card_history_photo1'] == ''){
		$strplace = 'card_history_photo1';
		$intkey = 1;
	}else if($arr['card_history_photo2'] == ''){
		$strplace = 'card_history_photo2';
		$intkey = 2;
	}else if($arr['card_history_photo3'] == ''){
		$strplace = 'card_history_photo3';
		$intkey = 3;
	}else if($arr['card_history_photo4'] == ''){
		$strplace = 'card_history_photo4';
		$intkey = 4;
	}else if($arr['card_history_photo5'] == ''){
		$strplace = 'card_history_photo5';
		$intkey = 5;
	}else{
		$intreturn = 2;
	}
...
function update_photo($objfile, $photo, $place, $fix = ''){
	...
	$strnewfile = $GLOBALS['intshop']."_".$GLOBALS['intid']."_".$fix.$strext;//新文件名
	$hresult = $objfile -> moveFile($strtmpfile, $strnewpath.$strnewfile, true);//move操作自带删除源文件
	if($hresult){
		$strsql = "UPDATE ".$GLOBALS['gdb']->fun_table2('card_history')." SET ".$place." = '".$strnewfile."' where card_history_id=".$GLOBALS['intid'];
