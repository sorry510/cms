...
foreach($arr as &$v){
	if($v['ticket_type']==1){
		$strsql = "SELECT ticket_money_name,ticket_money_value,ticket_money_limit FROM ".$GLOBALS['gdb']->fun_table2('ticket_money')." where ticket_money_id = ".$v['ticket_money_id']." limit 1";
		$hresult = $GLOBALS['gdb']->fun_query($strsql);
