...
$intreward_flag = $arrcompanyconfig['reward_flag'];

if($intreward_flag == 1){
	if($inttype == 1){
		$strsql1 = "SELECT worker_group_id FROM ".$gdb->fun_table2('group_reward2')." WHERE mgoods_id=".$intgoods_id." and group_reward2_money>0 and shop_id=".$GLOBALS['_SESSION']['login_sid'];
		$strsql = "SELECT worker_id,worker_name FROM ".$gdb->fun_table2('worker')." WHERE worker_group_id in (".$strsql1.")";
	}else if($inttype == 2){
