...
function get_reserve_list() {
	...
	$strsql = 'SELECT a.reserve_id,a.card_id,a.reserve_name,a.reserve_phone,a.reserve_count,a.reserve_state,a.reserve_atime,a.reserve_dtime,a.reserve_type,b.card_code FROM (SELECT reserve_id,card_id,reserve_name,reserve_phone,reserve_count,reserve_state,reserve_atime,reserve_dtime,reserve_type FROM ' . $GLOBALS['gdb']->fun_table2('reserve') . ' WHERE 1 = 1 ' . $strwhere .' ORDER BY reserve_dtime LIMIT ' . $intoffset . ', ' . $intpagesize . ') as a LEFT JOIN (SELECT card_code,card_id FROM ' . $GLOBALS['gdb']->fun_table2('card') .' WHERE card_state =1 AND card_edate >'. time() .' ) as b on a.card_id = b.card_id WHERE 1=1 ' . $strwhere2;
