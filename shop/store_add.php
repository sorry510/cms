...
/*function get_mgoods_list(){
	...
	$arr = $GLOBALS['gdb']->fun_fetch_all($hresult);
	foreach($arr as &$row){
		$strsql = "SELECT mgoods_id,mgoods_name, mgoods_price,mgoods_code,mgoods_jianpin FROM " . $GLOBALS['gdb']->fun_table2('mgoods')." WHERE mgoods_catalog_id = ".$row['mgoods_catalog_id']." and mgoods_type=2 and ORDER BY mgoods_id desc";
		$hresult = $GLOBALS['gdb']->fun_query($strsql);
...
/*function get_sgoods_list(){
	...
	$arr = $GLOBALS['gdb']->fun_fetch_all($hresult);
	foreach($arr as &$row){
		$strsql = "SELECT sgoods_id,sgoods_name, sgoods_price,sgoods_code,sgoods_jianpin FROM " . $GLOBALS['gdb']->fun_table2('sgoods')." WHERE sgoods_catalog_id = ".$row['sgoods_catalog_id']." and shop_id=".$GLOBALS['intshop']." and sgoods_type=2 ORDER BY sgoods_id desc";
		$hresult = $GLOBALS['gdb']->fun_query($strsql);
...
function get_sgoods_list(){
	$arr = array();
	$strsql = "SELECT a.*,b.sgoods_catalog_name FROM (SELECT sgoods_id,sgoods_name, sgoods_price,sgoods_code,sgoods_jianpin,sgoods_catalog_id FROM " . $GLOBALS['gdb']->fun_table2('sgoods')." WHERE sgoods_type=2 and shop_id=".$GLOBALS['intshop'].") AS a LEFT JOIN ".$GLOBALS['gdb']->fun_table2('sgoods_catalog')." AS b on a.sgoods_catalog_id = b.sgoods_catalog_id ORDER BY a.sgoods_catalog_id desc ";
	$hresult = $GLOBALS['gdb']->fun_query($strsql);
	$arr = $GLOBALS['gdb']->fun_fetch_all($hresult);
	return $arr;
}