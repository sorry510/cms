...
$strworker_group_id = api_value_post('id');
...
if($intreturn == 0){
	//删除本店的提成方案不包含sgoods
	$strsql = "DELETE FROM ".$gdb->fun_table2('group_reward')." where worker_group_id=".$intworker_group_id." and shop_id = ".$intshop;
	$gdb->fun_do($strsql);
	$strsql = "DELETE FROM ".$gdb->fun_table2('group_reward2')." where worker_group_id=".$intworker_group_id." and shop_id = ".$intshop." and sgoods_id=0";
	$gdb->fun_do($strsql);

	$strsql = "INSERT INTO ".$gdb->fun_table2('group_reward')." (worker_group_id,shop_id,group_reward_create,group_reward_fill,group_reward_pfill,group_reward_guide,group_reward_pguide,group_reward_atime) VALUES (".$intworker_group_id.",".$intshop.",".$arr['group_reward_create'].",".$arr['group_reward_fill'].",".$arr['group_reward_pfill'].",".$arr['group_reward_guide'].",".$arr['group_reward_pguide'].",".$intnow.")";
